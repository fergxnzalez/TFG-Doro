...
modelo_lstm_path = "modelo_lstm_0.h5"
...
if cap == 0:
    ...
    make_flag_wall(pos=wall_positions[0], ori=wall_orientations[0],
                        wall_half=[wall_thickness/2, room_size/2, wall_height/2],
                        stripes=sierraleone, axis='z')


    espania = [(1, "#00D0FF"), (2, "#FFFFFF"), (1, "#440092")]
    make_flag_wall(pos=wall_positions[1], ori=wall_orientations[1],
                        wall_half=[wall_thickness/2, room_size/2, wall_height/2],
                        stripes=espania, axis='z')
    lithuania = [(1, "#B4B102"), (2, "#035500"), (1, "#A00404")]
    make_flag_wall(pos=wall_positions[2], ori=wall_orientations[2],
                        wall_half=[wall_thickness/2, room_size/2, wall_height/2],
                        stripes=lithuania, axis='z')
    alemania = [(1, '#000000'), (1, '#DD0000'), (1, '#FFCE00')]
    make_flag_wall(pos=wall_positions[3], ori=wall_orientations[3],
                        wall_half=[wall_thickness/2, room_size/2, wall_height/2],
    ...
    p.createMultiBody(0, baseCollisionShapeIndex=p.createCollisionShape(p.GEOM_SPHERE, radius=0.9),
                         baseVisualShapeIndex=p.createVisualShape(p.GEOM_SPHERE, radius=0.9, rgbaColor=[0,0,1,1]),
...
ventana = 5
modeloLSTM = load_model(modelo_lstm_path, compile=False)
embeddings_buffer = []

def evaluar_si_hay_anomalia(nuevo_embedding):
    embeddings_buffer.append(nuevo_embedding)
    ...
    y_true = embeddings_buffer[2].reshape(1, -1)
    x_seq = x_seq.reshape(1, 4, -1)

    y_pred = modeloLSTM.predict(x_seq, verbose=0)

    # Error estilo MSE
    error = np.mean((y_true - y_pred) ** 2)
    print(f"MSE (real vs pred): {error:.6f}")
...
def calcular_embedding(image_route, image_name):
    imagen = Image.open(image_route).convert('RGB')
    ...
    # Guardar el vector en un .npy
    nombre_vector = os.path.splitext(image_name)[0] + ".npy"
    ruta_salida = os.path.join("vectores_eval", nombre_vector)
    np.save(ruta_salida, vector)

    return vector  # Devuelve el embedding
...
# Fuera del bucle
frame_imgs = [None, None, None, None]  # Para almacenar imágenes en memoria (frames 1-4)
img3_con_texto = None
last_error = None
frame_count = 0
...
while True:
    ...
    # Actualizar distancia ultrasónico
    ...
    # Actualizar frames almacenados en memoria si hay suficientes guardadas
    if frame_count > 3:
        # Solo actualizar lista en memoria cuando frame_count aumenta
        for i in range(4):
            ruta = f"frames_eval/FRAME-EVAL{frame_count - 4 + i:05d}.png"
            img = cv2.imread(ruta)
            if img is not None:
                frame_imgs[i] = img

    # Mostrar frames 1-4 (solo si existen)
    for i, f_img in enumerate(frame_imgs):
        if f_img is not None:
            if i == 2 and img3_con_texto is not None:
                # Mostrar versión con texto solo para frame 3
                cv2.imshow(f"FRAME {i+1}", img3_con_texto)
            else:
                cv2.imshow(f"FRAME {i+1}", f_img)

    cv2.imshow("FRAME ACTUAL", frame_img)

    # Guardar imagen y calcular embedding solo si pulsas tecla
    if is_key_pressed() is not None:
        image_route = f"frames_eval/FRAME-EVAL{frame_count:05d}.png"
        image_name = f"FRAME-EVAL{frame_count:05d}.png"
        cv2.imwrite(image_route, frame_img)
        frame_count += 1

        if frame_count > 4:
            embedding_real = calcular_embedding(image_route, image_name)
            error = evaluar_si_hay_anomalia(embedding_real)

            if error is not None and frame_imgs[2] is not None:
                errores_mse.append(error)
                linea_error.set_data(range(len(errores_mse)), errores_mse)
                ax.set_xlim(0, max(50, len(errores_mse)))
                ax.set_ylim(0, max(0.1, max(errores_mse) * 1.2))
                fig.canvas.draw()
                fig.canvas.flush_events()

                # Actualizar texto solo si cambia el error
                if last_error is None or abs(error - last_error) > 1e-6:
                    img3_con_texto = frame_imgs[2].copy()
                    cv2.putText(img3_con_texto,
                                f"ERROR: {error:.6f}",
